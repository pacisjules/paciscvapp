...
const projects = () => {
  ...
  const [gitlink, setgitlink] = useState(
    "https://github.com/pacisjules/reactjs_togglecss"
  );
  const [link, setlink] = useState(
    "https://loquacious-travesseiro-68f5c6.netlify.app/"
  );
  const [photo, setPhoto] = useState("/images/lightswitch.png");
  ...
  const see = () => {
    ...
    currentInfos.push(danne[lastObj]);

    console.log(currentInfos);
  };

  useEffect(() => {
    setgitlink("https://github.com/pacisjules/reactjs_togglecss");
    setlink("https://loquacious-travesseiro-68f5c6.netlify.app/");
    setPhoto("/images/lightswitch.png");
  }, []);
  ...
  const nextObject = () => {
    ...
    if (currentId > arrysize) {
      ...
      console.log(arrysize);

      const carray = [];
      const index = danne.filter((item) => {
        return item.id == currentId - 1;
      });
